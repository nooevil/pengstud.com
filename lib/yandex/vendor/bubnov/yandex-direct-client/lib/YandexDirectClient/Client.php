...
class Client {
    ...
    private static $mode = [
        'test' => 'https://api-sandbox.direct.yandex.ru/v4/json/',
        'live' => 'https://api.direct.yandex.ru/v4/json/'
    ];
    private static $modeType = 'test';
     ...
     * @param String $locale
     */
    public function __construct($token, $locale = 'ru', $mode = 'test') {
        $this->token = $token;
    ...
    private function request(\YandexDirectClient\Methods\AbstractMethod $method) {
        ...
        try {
            $response = $this->buzz->post(self::$mode[self::$modeType], self::$headers, json_encode($payload, JSON_UNESCAPED_UNICODE))->getContent();
        }
        catch (\Exception $e) {
